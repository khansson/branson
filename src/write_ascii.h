...
#ifndef write_ascii_h_
#define write_ascii_h_
...
void write_ascii(Mesh *mesh, const double& arg_time, const uint32_t& step,
  ...
  if (rank ==0) {
    ...
    if (ndims == 2) {
      ...
      FPT = fopen("output.vtk","w");
		...
		fprintf(FPT,"SCALARS cell_scalars float %d\n",1);
		fprintf(FPT,"LOOKUP_TABLE default\n");
		for (i = 0; i < n_xyz_cells; i++)
		{
			fprintf(FPT,"%f\n",T_e[i]);
		}
		fclose(FPT);
...
#endif // write_ascii_h_
//---------------------------------------------------------------------------//
// end of write_ascii.h
//---------------------------------------------------------------------------//
